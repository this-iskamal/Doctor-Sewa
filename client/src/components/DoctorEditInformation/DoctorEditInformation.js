...
import styles from "./DoctorEditInformation.module.css";
...
const DoctorEditInformation = () => {
  ...
  useEffect(() => {
    axios
      .get(`${baseurl}/doctor-edit-information/${id}`)
      .then((res) => {
        setUser(res.data.doctorinfo);
      });
  }, [id]);
  ...
  const handlenamechange = (e) => {
    ...
    if (username === "") toast.warn("Enter your name");
      ...
      axios
        .post(`${baseurl}/doctor-update-information/${id}`, {
          username,
        })
        .then((res) => {
  ...
  //     axios
  //       .post(
  //         `${baseurl}/doctor-update-information/${id}`,
  //         newdata
  //       )
  //       .then((res) => {
  //         console.log(res.data.message);
  ...
  const handlesendotp = (e) => {
    e.preventDefault();
    axios.post(`${baseurl}/doctor-send-otp/${id}`).then((res) => {
      toast.success(res.data.message);
    });
  };
  const handleconfirm = (e) => {
    ...
    e.preventDefault();
    axios
      .post(`${baseurl}/doctor-otp-confirm/${id}`, { password })
      .then((res) => {
        if (res.data.success) {
          toast.success(res.data.message);
          setTimeout(() => {
  ...
  return (
    <div className={styles.form}>
      <ToastContainer />
      <h1> Doctor Edit Information</h1>
      <h5 style={{color:'darkred'}}>(To change your speciality contact admin)</h5>
      ...
      <div className={styles.field}>
        <label htmlFor="address">Address : {user.address}</label>
        <div className={styles.sameline}>
          <select
